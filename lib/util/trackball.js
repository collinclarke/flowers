...
class TrackballControls extends THREE.EventDispatcher {
  constructor(object, domElement) {
    ...
    const getMouseOnCircle = ( function wrapper() {
      ...
      return (pageX, pageY) => {
        vector.set(
          ( ( pageX - _this.screen.width * 0.5 - _this.screen.left ) / ( _this.screen.width * 0.5 ) * 1),
          ( (( _this.screen.height + 2 * ( _this.screen.top - pageY ) ) / _this.screen.width) * 1) // screen.width intentional
        );
        // console.log(vector);
        return vector;
      };
