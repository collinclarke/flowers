...
class TrackballControls extends THREE.EventDispatcher {
  constructor(object, domElement) {
    ...
    const getMouseOnCircle = ( function wrapper() {
      ...
      return (pageX, pageY) => {
        vector.set(
          ( ( pageX - _this.screen.width * 0.5 - _this.screen.left ) / ( _this.screen.width * 0.5 )  * 0.15),
          ( ( _this.screen.height + 2 * ( _this.screen.top - pageY ) ) / _this.screen.width  * .15) // screen.width intentional
        );
        // console.log(vector);
        return vector;
      };
